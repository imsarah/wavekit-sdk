/**
 * Minimal ambient declarations for the optional peer dependencies the Tangem
 * adapter lazy-loads: `@walletconnect/sign-client` (always), plus
 * `ripple-binary-codec` / `xrpl` (only when the wallet returns a signed `tx_json`
 * that still needs to be serialized + hashed locally).
 */
...
declare module 'xrpl' {
  export const hashes: {
    hashSignedTx(tx: string | Record<string, unknown>): string;
  };
}
